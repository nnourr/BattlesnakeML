...
class Model:
  ...
  def train_SVC(self):
    ...
    self.model = svc.fit(self.data.lda_2, self.data.encoded_moves)
    return svc
    
  def train_Decision_Tree(self):
    dtc = DecisionTreeClassifier()
    self.__cross_validate(dtc)
    self.model = dtc.fit(self.data.lda_2, self.data.encoded_moves)
    return dtc
  ...
  def train_neural_network(self):
    ...
    for train, test in kfold.split(inputs, outputs):
      inputs_train = inputs[train].reshape(-1, 2)  # Each sample has 2 features
      inputs_test = inputs[test].reshape(-1, 2)  # Each sample has 2 features

      # Define the model architecture
      model = Sequential()
      model.add(Input(shape=(2,)))  # Input shape is (2,) since each sample has 2 features
  ...
  def __cross_validate(self, model):
    results = cross_validate(model, self.data.lda_2, self.data.encoded_moves, cv=10, scoring='balanced_accuracy')
    print(results)
